...
if [ -z "$(git config user.name)" ] || [ -z "$(git config user.email)" ]; then
    exit_error "Cannot install: You need to set your username and email address in Git, like so:" \
...
self_update() {
    ...
    if git diff --name-only "$current_commit..HEAD" | grep -q -E "$this_script_name|$COMMON_SCRIPT"; then
